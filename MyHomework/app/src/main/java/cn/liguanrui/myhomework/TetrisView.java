...
public class TetrisView extends View {
    ...
    public static int beginPoint = 50;  //网格开始坐标值，横纵坐标的开始值都是此值

    public static int max_x, max_y;     //保存俄罗斯方块单元的最大横纵坐标

    public static float beginX;         //俄罗斯方块初始x坐标

    public int dropSpeed = 500;         //俄罗斯方块下落线程默认休眠时间
            ...
            public void run() {
                while(isRun) {
                    try {
                        ...
                        h = getHeight();
                        w = getWidth();
                        beginX  = (int)((w -  beginPoint)/100) * 50 - beginPoint/2 +25;
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    if(flag) {
                    ...
                    if(thread.getState() == Thread.State.TERMINATED || thread.getState() == Thread.State.NEW) {
                        ...
                        thread = new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                int end = (int) ((h - 100 - beginPoint) / BlockUnit.UNITSIZE);
                                ...
                                for (BlockUnit u : temp.getUnits()) {
                                    ...
                                    int index = (int) ((u.getY() - beginPoint) / 100); //计算所在行数
                                    map[index]++;
                                }

                                //每行最大个数
                                int full = (int) ((w - 100 - beginPoint) / BlockUnit.UNITSIZE) + 1;
                                    ...
                                    if (map[i] >= full) {
                                        ...
                                        for (int j = blocks.size()-1; j>=0; j--) {
                                            ...
                                            for (BlockUnit u : blocks.get(j).getUnits()) {
                                                if ((int) ((u.getY() - beginPoint) / 100) < i)
                                        ...
                                        father.runOnUiThread(new Runnable() {
                                            ...
                                            public void run() {
                                                ...
                                                father.speed.setText(father.playerString + father.newname);
                ...
                if(isRun == false){
                    ...
                    father.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(father,"游戏结束！", Toast.LENGTH_SHORT).show();
                        }
                    });
    ...
    public void onDraw(Canvas canvas){
        ...
        //绘制网格
        for(int i=beginPoint; i<max_x-100; i+= 100){
            for(int j=beginPoint; j<max_y-100; j+= 100) {
                rel = new RectF(i, j, i + 100, j + 100);
                canvas.drawRoundRect(rel, 8, 8, paint);
            }
        }
    }
